...
struct JWTPayload {
    ...
    JWTPayload(const string& username) {
...
// URLセーフなBase64エンコード
inline string base64url_encode(const string& input) {
...
// URLセーフなBase64デコード
inline string base64url_decode(const string& input) {
...
// JSONエンコード（簡易版）
inline string json_encode(const JWTPayload& payload) {
    stringstream ss;
    ss << "{";
    ss << "\"iss\":\"" << payload.iss << "\",";
    ss << "\"sub\":\"" << payload.sub << "\",";
...
// JWTトークン生成
inline string generate_token(const string& username, const string& secret_key) {
...
// JWTトークン検証
inline bool verify_token(const string& token, const string& secret_key) {
...
// JWTペイロードデコード
inline JWTPayload decode_payload(const string& token) {
    size_t first_dot = token.find('.');
    size_t second_dot = token.find('.', first_dot + 1);
...
// トークンの有効期限チェック
inline bool is_token_expired(const string& token) {
