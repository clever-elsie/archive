...
Entry詳細は最小構成とし、子Entryは`children`で取得する。ディレクトリ表示は`children?all=true`で現在ディレクトリの到達可能な直下の子を一覧し、`page`とは別の表示モードとして同じ一覧コンテナを排他的に利用する。`attached_media: true`のWorkはCollection直下のメディアを閲覧するための子Entryであり、通常のディレクトリWorkではない。これは`children`では取得できるが、検索・random・`page`・タグ編集の対象外である。`page`は通常のWorkとmixed Work内の独立Memberを対象にした、全メディアまたはメディア種別ごとの更新時刻順キャッシュをページングする。ただし動画葉を持つWorkは、`filter=all`でも親Workではなく動画葉から生成された各MediaSetを1件として扱う。したがって複数の動画葉を持つWorkも全メディアページと動画ページの両方で葉ごとに分かれ、単一動画の葉も同じ対象になる。`page`の`filter=all|image|video|audio|text|document`はキャッシュを選び、既定の更新日時降順ではキャッシュ順をそのまま使う。名前順や更新日時昇順を要求した場合だけ、選択された全キャッシュをその順に並べ替えてからページングする。検索は全Workを逐次走査して`page`、`limit`、`sort_key=path|updated_at`、`direction=asc|desc`、`filter`で絞り込む。検索語は名前・相対パス・タグを大文字小文字を区別しない部分一致で評価し、`tag:語`はタグだけ、`path:語`は相対パスだけに対象を限定する。空白はAND、`AND`／`OR`／`NOT`、`!`、`&`／`|`、括弧、`"複数語"`による引用を旧検索と同じ文法で利用できる。例として`tag:猫 path:漫画`は両方を満たすWork、`(猫 OR path:book) NOT tag:除外`は括弧内に一致し、除外タグを持たないWorkを返す。不正な括弧や引用符、空のフィールド指定は`INVALID_QUERY`を返す。`random`はページングせず、指定種別のキャッシュから少数の固定個数を無作為に返す。`grouping=mixed`は全種別を一列で比較し、`grouping=media_type`はディレクトリ、画像、動画、音声、テキスト、ドキュメントの固定グループ順で比較する。グループ内だけ方向指定を反映する。WorkとCollectionの`media_types`は要求者から到達可能な子だけを集約した種別要約である。Workを開いたときの初期MediaSetは要求された順序の先頭である。ページング／randomでMemberまたは動画葉のMediaSetが返された場合も、`parent_id`からMediaSetとWorkを解決して通常のWork表示を開く。更新時刻も精度を失わない文字列で表現する。`display_name`は有効な末尾ルビ注記を除いた表示本体、`display_name_ruby`はルビ文字列であり、HTMLは返さない。管理者の開発者メニューは`include_hidden=true`を付け、`hidden_aliases`を表示できる。
...
reloadは管理者だけが明示的に実行でき、最低待機時間未満の要求は`RELOAD_COOLDOWN`、実行中の重複要求は`RELOAD_ALREADY_PENDING`として返す。dirtyはfilesystemまたはメタデータの再走査が必要であることを示す無効化ラッチであり、設定された定期走査時刻までreloadを予約しない。定期走査時に自動reloadを一度だけ予約し、reload開始時点までに発生したdirtyをその走査へ含める。走査開始後から公開完了までに発生したdirtyはフラグを保持し、次の定期走査で一度だけ処理する。複数回のdirtyは一つに畳み込む。reload失敗時もdirtyを保持して次の定期走査で再試行する。タグの追加・削除はcurrent GraphStateと`.viewer.json`へ直接反映し、成功時にreloadを予約しない。ファイルシステムへの反映に失敗した場合だけdirtyを立てる。定期スキャン間隔は`VIEWER_SCAN_INTERVAL_SECONDS`で秒単位に指定し、未指定時の既定値は3時間（10800秒）とする。0以下は設定エラーとする。
...
初回GraphStateがない起動時だけ画面全体をサービス停止表示にし、reload中は古いEntry、CollectionContainer、MediaSetsContainer、メインメディアを保持する。フロントエンドは一覧ごとに独立したsort、direction、grouping、filter、page状態を持ち、自動移動、メディア終了時の再生モード（ループ・自動遷移・何もしない）、音量、MediaMembersの開閉、現在のEntry IDも再読み込み後に復元する。内容一覧は表示行数も保持し、0は横長viewportで2行・縦長viewportで4行の自動設定、正の整数は指定行数、-1は上限なしとして扱う。MediaSetsはAPIから全件取得し、内容一覧と同じ表示行数設定からクライアント側のページサイズを計算して切り替える。MediaMembersはpage状態を持たず、全件取得してコンテナ内をスクロールする。横長viewportではメイン表示とMediaMembersを横並びにし、MediaSetsをその下、Collectionの上に置く。縦長viewportでもMediaSetsはメイン表示の下に置き、MediaMembersだけを左右ドロワーにしてメイン表示が全幅を使う。Entry IDはDOMやURLで文字列として扱い、64bit数値へ変換しない。作品選択はメインコンテンツの中央へ、Collection選択は`#THUM`へ移動する。ディレクトリ表示とページングは同じ内容コンテナを共有するが、同時には表示しない。ディレクトリ表示ではAPIから取得した直下の全件をクライアント側に保持し、ディレクトリEntryは常に全件を表示したまま、葉Entryだけを表示行数と列数から作ったページに分けて切り替える。-1では葉Entryも全件を1ページに表示する。ページング、検索、randomの取得数は固定値にせず、取得直前に画面の幅とグリッド列数、内容一覧の表示行数から計算する。ページングが表示中は左右矢印キーでも前後ページへ移動できるが、入力欄・音声・動画操作中のキー入力は奪わない。
